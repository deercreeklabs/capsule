...
(defn send-rpc [tube-client api rpc *rpc-id-str->rpc]
  (let [{:keys [rpc-name rpc-id rpc-id-str arg cb]} rpc]
    (tc/send tube-client (u/encode api {:msg-type :rpc-req
                                        ...
                                        :msg-id rpc-id
                                        :msg arg}
                                   (u/sym-map msg-type msg-name msg-id msg)))
  ...
  (send-rpc [this rpc-name arg cb timeout-ms]
    (if @*shutdown?
      ...
      (let [timeout-ms (max timeout-ms max-rpc-timeout-ms)
            ...
            rpc-id-str (ba/byte-array->b64-string rpc-id)
        ...
        (enqueue-rpc rpc-chan rpc max-rpc-timeout-ms cb))))
...
(defn <log-in [tube-client rcv-chan api subject-id credential]
  (au/go
    (let [msg-id (u/make-msg-id)
          msg (u/sym-map subject-id credential)
          _ (send-msg tube-client api :login-req :login msg-id msg)
          data (ca/<! rcv-chan)
          rsp (u/decode api data)
          {:keys [msg-type msg]}]
        ...
        (try
          ...
          (catch #?(:clj Exception :cljs js/Error) e
              (errorf "Unexpected error in rpc-loop: %s"
                      (tu/get-exception-msg-and-stacktrace e))))))))
    ...
    (while (not @*shutdown?)
      (try
        (doseq [[rpc-id-str rpc] @*rpc-id-str->rpc]
          (let [{:keys [retry-time-ms failure-time-ms rpc-id-str]} rpc
                now (u/get-current-time-ms)]
            (cond
              (> now failure-time-ms)
              (do
...
(defn start-rcv-loop
  [rcv-chan api *client-schema-pcf *shutdown? *rpc-id-str->rpc]
  (ca/go
    (while (not @*shutdown?)
      (try
        (let [[data ch] (ca/alts! [rcv-chan (ca/timeout 100)])]
          (when (= rcv-chan ch)
            (let [msg-info (u/decode api @*client-schema-pcf data)]
...
(s/defn make-client :- (s/protocol ICapsuleClient)
  ...
  ([api :- u/Api
   ...
   (let [opts (merge default-client-options opts)
         ...
         rpc-chan (ca/chan (math/ceil (* max-msgs-per-sec rpc-burst-seconds
         ...
         *rpc-id-str->rpc (atom {})
         client (->CapsuleClient
                 ...
                 max-total-rpc-time-ms
                 *event-name->handler *shutdown? *rpc-id->cb)]
